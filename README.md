...
go-valuate/
  ...
  accounting/labor/         deterministic payroll/labor analytics from a portable payroll-register/workforce-roster/contractor-spend model (independent of accounting/ledger, accounting/statements, accounting/ar, accounting/ap, accounting/cashforecast, and analytics/*): labor-cost bridge (regular/overtime/bonus/commission/other pay, employer taxes/benefits/other burden, contractor labor), cash-vs-expense basis, headcount/FTE (hours-based or caller-supplied), overtime and contractor mix, direct/indirect labor split, department/location/cost-center summaries, revenue/gross-profit/EBITDA-per-employee-or-FTE productivity, labor-cost-vs-revenue trend flags, workforce movement/turnover, worker-date consistency review, possible-duplicate-record detection, payroll-register-to-GL reconciliation, portable financial-metrics/ledger/cashforecast adapters demonstrated via tests — not payroll processing (no tax withholding/gross-to-net/remittances/benefits administration), no individual-worker ranking or termination recommendations (see docs/LABOR_ANALYTICS.md)
  accounting/inventory/    deterministic inventory analytics from a portable item/snapshot/movement model, converging with a caller-pre-aggregated period-summary path (independent of accounting/ledger, accounting/statements, and analytics/*): on-hand quantity/value (quantity x cost or explicit value, never silently choosing between disagreeing figures), turnover/DIO (never Inf at zero COGS), value aging with an explicit UNKNOWN bucket and slow/non-moving classification (no invented obsolescence period), last-movement/velocity/weeks-of-supply, caller-supplied stock-policy (min/target/max/reorder) comparison with no inferred over/understock claim, inventory value concentration by item/category/location (reuses analytics/concentration), purchases-vs-usage and inventory-build signals, adjustment/write-off review (neutral language, no shrinkage/fraud inference), expiry review, multi-component GL/subledger reconciliation, quantity/value rollforwards, explicit UOM/currency safety, portable statements/working-capital/ratios adapters demonstrated via tests — not an ERP/WMS/costing engine, no FIFO/LIFO/tax-inventory accounting, no automatic write-down journal entries, no product profitability (see docs/INVENTORY_ANALYTICS.md)
  accounting/profitability/ deterministic customer/job/product profitability engine analyzing one economic-fact population through three independent dimensions (CUSTOMER/JOB/PRODUCT; no compile-time dependency on accounting/ledger, accounting/statements, accounting/labor, accounting/inventory, or analytics/*): one-fact/multi-view attribution (never duplicated per dimension), partial attribution with explicit unattributed remainder (never renormalized), profitability bridge (net revenue -> gross profit -> contribution profit -> allocated profit), opt-in shared-cost-pool allocation with independent per-dimension basis/rules and full trace/reconciliation (no default basis, no equal-allocation fallback on a zero denominator), business totals computed directly from facts (never by summing dimension views — no cross-dimension double counting), control-total reconciliation, rankings/margin-leakage/trend analytics (no composite score, no prediction), portable statements/labor/inventory/ledger/concentration adapters demonstrated via tests — not a pricing optimizer, ERP, CRM, or AI recommendation engine, no customer-termination/product-discontinuation recommendations (see docs/PROFITABILITY_ANALYTICS.md)
  accounting/vendorspend/  deterministic economic supplier spend analytics from a portable purchase-record model (independent of accounting/ap, accounting/inventory, accounting/ledger, accounting/statements, and analytics/*; distinct from accounting/ap's open-AP-balance question — see docs/VENDOR_SPEND_ANALYTICS.md's AP-vs-spend section, locked by a permanent regression test): gross/credit/net spend bridge, supplier/category/department/location/cost-center summaries, supplier spend concentration (reuses analytics/concentration; never equated with vendor dependency), caller-declared dependency metadata (never inferred from spend share), new/lost-supplier detection, growth/share trends, quantity/UOM-safe unit-price analytics and cross-supplier price comparison, observed-single-source product detection, price-volume decomposition (locked identity), caller-declared and pattern-observed recurring spend, committed/discretionary mix, caller-defined tail spend, bounded-indexed possible-duplicate-spend detection (no O(N^2) scan, neutral language only), preferred/contracted-supplier policy flags, control-total reconciliation (offsetting differences stay visible), category/product/metadata coverage, portable AP/inventory/labor/profitability adapters demonstrated via tests — not a procurement workflow, PO/approval, vendor onboarding, payment execution, contract management, supplier credit scoring, compliance screening, or AI recommendation engine (see docs/VENDOR_SPEND_ANALYTICS.md)
  accounting/reconciliation/ deterministic account reconciliation between book-side and external/control-side records (one engine, nine ReconciliationType labels — BANK/CREDIT_CARD/LOAN/AR_CONTROL/AP_CONTROL/INVENTORY_CONTROL/PAYROLL_CLEARING/INTERCOMPANY/SUSPENSE_CLEARING/GENERIC — metadata only, never a parallel code path; shares only a leaf package name with the pre-existing, unrelated financial/reconciliation): explicit INFLOW/OUTFLOW signed-amount convention with caller-declared SAME/REVERSED orientation applied to both transactions and balances, balance-only and transaction-level modes kept strictly separate, indexed (not O(N^2)) matching precedence (reference+amount+date -> amount+date -> amount+window), bounded opt-in one-to-many/many-to-one composite matching with an explicit combination budget, caller-confirmed manual matches at highest precedence (many-to-many only reachable this way), strict never-guess ambiguity handling, caller-supplied reconciling items (no auto-generation, no journal posting) driving one explicit reconciliation equation, opening+activity=ending rollforward with supplied-vs-derived-balance safety, factual coverage/match-quality summaries (no composite score), generic staleness aging (distinct from accounting/ar's/accounting/ap's own aging), ledger/AR/AP/inventory/labor/closequality adapters proven against real sibling fixtures/results, closequality-critical-account integration proven both ways (reconciled -> doesn't block, unreconciled -> NOT_READY), neutral language only, a real O(N^2) matcher bug found+fixed via profiling (~225x faster at 10k/10k scale) — not bank-feed ingestion, OCR, journal posting, payment execution, workflow, fuzzy/AI matching, or fraud detection (see docs/ACCOUNT_RECONCILIATION.md)
  accounting/closechecklist/ deterministic period-close checklist and close-readiness domain engine (consumes accounting/closequality.Result and accounting/reconciliation.Result as typed gate facts, never recomputing their diagnostics — distinct question from closequality's "are there control issues" and reconciliation's own matching): reusable versioned Template (sections/tasks, separate TemplateContractVersion), caller-defined-opaque section/task codes, ALWAYS/CALLER_FLAG/EXTERNAL_GATE_PRESENT applicability with explicit overrides (no expression engine), MUST_BE_COMPLETED/MUST_BE_RESOLVED task dependencies with iterative (non-recursive) cycle detection that is never silently broken, order-independent dependency-respecting evaluation, NONE/AT_LEAST_ONE/MIN_COUNT/SPECIFIC_TYPES evidence policies, NONE/PREPARER_ONLY/PREPARER_AND_REVIEWER/SPECIFIC_ROLES sign-off review policies with distinct-actor role-conflict detection, calendar-day PERIOD_END/TARGET_CLOSE_DATE/EXPLICIT_DATE due rules against a required explicit EvaluationDate (never time.Now()), PASS/PASS_OR_WARNING/AVAILABLE external gate rules (a missing required gate is never implicitly PASS), NOT_ALLOWED/ALLOWED/ALLOWED_WITH_APPROVAL exceptions that suppress effective blocking without ever deleting the underlying blocker, "completed but missing requirements" detection, section/completion/evidence/sign-off/gate coverage summaries (no composite score), typed explainable blockers, period-state consistency checking (never mutating caller state), prior-close and template-version comparison, ServiceBusinessMonthlyClose/InventoryBusinessMonthlyClose example templates, neutral language only, a real O(N^2) downstream-dependency-count bug found+fixed via a bitset dynamic-programming rewrite (~190x faster at 5,000-task chain scale) — not persistence, workflow/task-assignment, actual period locking, journal posting, reconciliation matching, or AI (see docs/PERIOD_CLOSE_CHECKLIST.md)
  ...
  analytics/kpi/            safe, deterministic, reusable KPI engine (independent of every accounting/analytics package at compile time): typed-AST formulas (no eval/scripting/reflection/SQL), known-zero-vs-unavailable metric inputs, small explicit unit-compatibility system (never hard-codes USD), strict-by-default dimensions (no implicit broadcast), KPI-to-KPI dependency graph with cycle/namespace-collision detection, caller-defined targets/threshold-bands/trends, optional calculation trace + always-on provenance, 8 portable module adapters (financial/labor/ar/ap/inventory/profitability/vendorspend/cashforecast) that republish existing computed values rather than recomputing formulas, 6 built-in templates evaluated by the identical engine (no special-casing) — not a scripting/expression-language runtime, no string DSL in V1 (see docs/KPI_ENGINE.md)
...
| Labor analytics schema | `labor.SchemaVersion`, echoed on `Result.SchemaVersion` | The `Worker`/`PayrollRecord`/`ContractorLaborRecord`/`PeriodInfo`/`BusinessMetrics`/`GLPayrollControl`/`Policy`/`PeriodSummary`/`GroupSummary`/`TrendResult`/`ReconciliationSummary`/`Flag`/`Issue`/`Coverage`/`Result` shapes (`accounting/labor`); see `docs/LABOR_ANALYTICS.md` |
| Labor analytics formulas | `labor.FormulaVersion`, echoed on `Result.FormulaVersion` | The labor-cost bridge (`bridge.go`), cash-vs-expense basis (`basis.go`), headcount/FTE (`headcount.go`/`fte.go`), overtime (`overtime.go`), contractor mix (`contractor.go`), direct/indirect and department/location/cost-center grouping (`grouping.go`), productivity ratios (`productivity.go`), trend/growth calculation (`trend.go`), worker-date consistency (`workforcedates.go`), duplicate detection (`duplicates.go`), payroll-vs-GL reconciliation (`reconcile.go`), and every flag-trigger rule (`computeflags.go`) (`accounting/labor`) |
| Inventory analytics schema | `inventory.SchemaVersion`, echoed on `Result.SchemaVersion` | The `Item`/`InventorySnapshot`/`Movement`/`StockPolicy`/`PeriodInfo`/`PeriodFinancials`/`GLControl`/`Policy`/`PeriodSummary`/`ItemAging`/`AgingSummary`/`ReconciliationSummary`/`Flag`/`Issue`/`Coverage`/`Result` shapes (`accounting/inventory`); see `docs/INVENTORY_ANALYTICS.md` |
| Inventory analytics formulas | `inventory.FormulaVersion`, echoed on `Result.FormulaVersion` | Value/quantity resolution (`value.go`), turnover/DIO (`turnover.go`), aging buckets and slow/non-moving thresholds (`aging.go`), last-movement/velocity (`aging.go`/`velocity.go`), stock-policy comparison (`stockpolicy.go`), concentration (`concentration.go`), purchases-vs-usage and inventory-build signals (`usage.go`), adjustment/write-off review (`adjustments.go`), expiry review (`expiry.go`), reconciliation and rollforwards (`reconcile.go`), and every flag-trigger rule (`flags.go`) (`accounting/inventory`) |
| Profitability analytics schema | `profitability.SchemaVersion`, echoed on `Result.SchemaVersion` | The `Entity`/`Fact`/`Attribution`/`PeriodInfo`/`EntityPeriodSummaryInput`/`DriverObservation`/`SharedCostPool`/`AllocationRule`/`ControlTotals`/`Policy`/`EntityPeriodResult`/`AllPeriodResult`/`DimensionView`/`BusinessTotals`/`Flag`/`Issue`/`Coverage`/`Result` shapes (`accounting/profitability`); see `docs/PROFITABILITY_ANALYTICS.md` |
| Profitability analytics formulas | `profitability.FormulaVersion`, echoed on `Result.FormulaVersion` | The profitability bridge (`bridge.go`/`entityresult.go`), attribution resolution (`attribution.go`), allocation math (`allocation.go`), coverage (`coverage.go`), rankings (`rankings.go`), trend/margin-leakage (`trend.go`), and every flag-trigger rule (`computeflags.go`) (`accounting/profitability`) |
| Vendor spend analytics schema | `vendorspend.SchemaVersion`, echoed on `Result.SchemaVersion` | The `Supplier`/`Period`/`SpendRecord`/`ControlTotals`/`Policy`/`Result` and every sub-shape (`accounting/vendorspend`); see `docs/VENDOR_SPEND_ANALYTICS.md` |
| Vendor spend analytics formulas | `vendorspend.FormulaVersion`, echoed on `Result.FormulaVersion` | The gross/credit/net bridge (`bridge.go`), supplier/category/period summaries (`summaries.go`/`categories.go`), concentration (`concentration.go`), new/lost-supplier and growth/share trends (`trends.go`), unit-price and cross-supplier price analytics (`unitprice.go`), price-volume decomposition (`pricevolume.go`), recurrence/commitment mix (`recurrence.go`), tail spend (`tailspend.go`), possible-duplicate-spend detection (`duplicates.go`), preferred/contracted-supplier policy (`supplierpolicy.go`), control-total reconciliation (`reconcile.go`), coverage (`coverage.go`), and every flag-trigger rule (`flags.go`) (`accounting/vendorspend`) |
| KPI engine schema | `kpi.SchemaVersion`, echoed on `Result.SchemaVersion` | The `Definition`/`Expression`/`MetricRef`/`KPIRef`/`Unit`/`MetricValue`/`DimensionKey`/`Period`/`TargetPolicy`/`ThresholdBand`/`Options`/`KPIResult`/`Trace`/`Provenance`/`Result`/`Coverage`/`DefinitionIssue`/`EvaluationIssue` shapes (`analytics/kpi`); see `docs/KPI_ENGINE.md` |
| KPI engine formulas | `kpi.FormulaVersion`, echoed on `Result.FormulaVersion` | Every `Operator`'s exact semantics (`arithmetic.go`), unit-compatibility rules (`unit.go`), aggregation-rule semantics (`aggregate.go`), target/band evaluation (`target.go`/`band.go`), trend/change computation (`trend.go`), and dependency-cycle/validation rules (`dependency.go`/`validate.go`) (`analytics/kpi`) |
| KPI expression-language version | `kpi.ExpressionLanguageVersion`, echoed on `Result.ExpressionLanguageVersion` | Whether a given `Expression` tree shape (independent of what it computes) is one this engine's validator/evaluator accepts at all — a distinct axis from schema (field shapes) and formula (operator semantics); see `docs/KPI_ENGINE.md`'s expression-language-versioning section (`analytics/kpi`) |
| Account reconciliation schema | `reconciliation.SchemaVersion`, echoed on `Result.Versions.SchemaVersion` | The `Input`/`MatchingPolicy`/`BookItem`/`ExternalItem`/`MatchGroup`/`Candidate`/`ReconcilingItem`/`Balance`/`Coverage`/`MatchQualitySummary`/`Finding`/`Issue`/`Result` shapes (`accounting/reconciliation`); see `docs/ACCOUNT_RECONCILIATION.md` |
| Account reconciliation formulas | `reconciliation.FormulaVersion`, echoed on `Result.Versions.FormulaVersion` | The reconciliation equation and balance-derivation/rollforward formula (`equation.go`/`balances.go`), and the status decision table (`status.go`) (`accounting/reconciliation`) |
| Account reconciliation matching version | `reconciliation.MatchingVersion`, echoed on `Result.Versions.MatchingVersion` | The matching-precedence order and indexed-lookup strategy (`matcher.go`) and the bounded composite-search algorithm/limits (`composite.go`) — kept separate from the formula version because a persisted auto-match decision may need to evolve independently of the equation/status arithmetic itself (`accounting/reconciliation`) |
...
| `reconciliation.Result.MatchedGroups` | Confirmed matches first (by construction, `MatchReason == CONFIRMED` always ranks first), then by matching-precedence rank (reference+amount+date, then amount+date, then amount+window, then composite), then by `DateDifferenceDays` ascending, then by the group's first `BookItemID` (or first `ExternalItemID` if it has none) — a real `sort.SliceStable` (`reconciliation`'s `sortMatchGroups`), with `MatchID`s ("M001", "M002", ...) assigned only after this final order is fixed (`accounting/reconciliation`) |
| `reconciliation.Result.UnmatchedBookItems` / `UnmatchedExternalItems` | Lexically sorted by `ItemID` (`accounting/reconciliation`) |
| `reconciliation.Result.AgedUnmatchedBookItems` / `AgedUnmatchedExternalItems` / `AgedReconcilingItems` | By `DaysOutstanding` descending (most stale first), then by `ItemID` ascending (`accounting/reconciliation`) |
| `reconciliation.Result.Findings` | By `Severity` (WARNING before INFO), then by `FindingCode`, then by the finding's first `ItemID` (`accounting/reconciliation`'s `sortFindings`) |
| `reconciliation.Result.Issues` | By `Severity` (error before warning), then by `IssueCode`, then by `ItemID` (`accounting/reconciliation`'s `sortIssues`) |
