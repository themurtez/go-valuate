...
// sales rep) aggregated aging figures — section 26.
...
// present on at least one included Receivable.
...
// key, sorted by OpenAmount descending then Value ascending (deterministic
// — section 35). This is a single caller-selected grouping, not arbitrary
// OLAP/cube logic, per the task's explicit scope limit.
func buildDimensionSummary(rows []receivableAging, sortedBuckets []BucketDefinition, key string) DimensionSummary {
	if key == "" {
	...
	for _, row := range rows {
		...
		v, ok := dimensionValue(row.r.Dimensions, key)
		if !ok {
			continue
		}
		a, exists := byValue[v]
	...
	for _, row := range rows {
		...
		if !bucketIsCurrent(sortedBuckets, row.bucketCode) {
			overdueByValue[v] += row.r.OpenAmount
		}
	}
