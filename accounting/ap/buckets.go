...
// AgingByDueDate — due-date aging measures actual delinquency relative to
// the contractual payment obligation, while bill-date aging conflates long
// payment terms with lateness. Mirrors accounting/ar.AgingBasis's identical
// rationale.
...
const (
	...
	// AgingByBillDate ages AsOfDate - BillDate, for a caller who
	// specifically wants bill-age rather than delinquency.
	AgingByBillDate AgingBasis = "AGING_BY_BILL_DATE"
)

// resolvedAgingBasis returns b if recognized, otherwise AgingByDueDate.
func resolvedAgingBasis(b AgingBasis) AgingBasis {
	if b == AgingByBillDate {
		return b
	}
...
type BucketDefinition struct {
	...
	// label — Code is never inferred from Label. Required, unique within
	// a BucketDefinition slice.
...
// computation and SupplierSummary bucket breakdown uses, independent of
// caller input order.
...
// package requires: no gaps (unless AllowBucketGaps), no overlap, exactly
// one terminal open-ended bucket, unique codes, non-negative bounds, and
...
// before this is invoked — see ageDays) always matches the first bucket.
// Returns ("", false) only if defs is empty or daysPastDue falls in an
// unresolvable gap (AllowBucketGaps was used and no bucket covers this
// value).
...
// the literal string "CURRENT" as a semantic identifier.
...
// within sortedBuckets, for comparing "did this payable get worse or
// better."
