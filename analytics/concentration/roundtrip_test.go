...
// (History, Trends, DependencyChanges, Scenarios, Flags) — mirroring
// revenuequality/roundtrip_test.go.
func TestResult_JSONRoundTrip(t *testing.T) {
	rate := 0.3
	res := Calculate(Input{
		Basis:        BasisSupplierSpend,
		Observations: highlyConcentratedObservations(),
		PeriodMeta:   threeYearMeta(),
		Policy: Policy{
			ScenarioTopN:            []int{1, 2, 3},
			DefaultImpactMarginRate: &rate,
			EntityImpactAssumptions: []EntityImpactAssumption{
				{EntityKey: "whale", MarginRate: 0.45},
			},
		},
	}, Options{})
	if !res.Available {
